...
`immediate` is supported by [useMount](/reference/use-mount).
...
By using `compare`, you can customize the logic for data updates to prevent frequent refreshes. This is particularly useful in dealing with some **pseudo-differences** in data, such as timestamps. It can reduce unnecessary rendering, supported by [useAsyncFn](/reference/use-async-fn).

```ts
const { loading, data, error } = useQuery(fetchData, {
  compare: (preData, nextData) => {
    // Compare whether the data is the same, return true to not update, return false to update, shallowEqual is used by default
