...
// Manually triggered, which will trigger the update of the params state
run('newParams')

// Use the last parameters to make the request again, equivalent to `run(params)`
refresh()
...
### Parameters and Refresh \{#parameters-refresh}

Using `params` can get the parameters of the last request, using `refresh` can make a request again with the last request's parameters, supported by [useAsyncFn](/reference/use-async-fn).
