...
这是一个 Util，提供一种通过多个 Hooks 或简单的 JS/TS 函数管理单个（或全局）加载状态的方法，底层使用了 [Reactive](https://sheinsight.github.io/reactive/)。
...
export interface CreateSingleLoadingReturns {
  ...
  /**
   * 通过在 JS/TS 中的 store 获取加载状态
   *
   * @returns {boolean} `boolean`, 加载状态
   */
  get(): boolean
  /**
   * 将加载状态绑定到 JS/TS 中的异步函数
   *
   * @param {AnyFunc} asyncFn - `AnyFunc`, 要绑定的异步函数，见 {@link AnyFunc}
