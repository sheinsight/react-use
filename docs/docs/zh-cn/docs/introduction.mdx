...
`@shined/react-use` 是一种重塑 React 开发的新编程范式，本质上是一个对 **SSR（服务端渲染）友好**、**全面**且**高度优化**的 React Hooks 库，提供了灵活且高效的钩子解决方案。完全采用 **TypeScript** 开发，配备了包含丰富示例的交互式文档 🔥。
...
- **灵活性**：特性包括 [依赖收集 Dependencies Collection](/docs/features/dependencies-collection)、[元素目标 ElementTarget](/docs/features/element-target)、[Ref 获取函数 Ref Getter](/docs/features/ref-getter)、[可暂停 Pausable](/docs/features/pausable) 等。
